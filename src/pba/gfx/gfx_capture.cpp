...
auto GfxContext::capture_viewport_rgba8(std::vector<u8>& out) const -> bool
{
    Expects(loaded_glad);
    if (!loaded_glad)
    {
...
auto GfxContext::start_recording() -> void
{
...
auto GfxContext::stop_recording() -> void
{
    if (!recorder.is_recording())
    {
        return;
...
auto GfxContext::toggle_recording() -> void
{
    if (recorder.is_recording())
    {
